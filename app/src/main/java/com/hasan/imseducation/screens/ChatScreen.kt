...
import com.hasan.imseducation.api.Firebase
...
//                    }
            ...
            colors = CardDefaults.cardColors(if (fromMe) blue else gray)
        ) {
            Text(
                text = message.text!!,
                color = Color.White,
        ...
        trailingIcon = {
            IconButton(
                ...
                Icon(
                    Icons.Rounded.Send,
                    contentDescription = "",
                    tint = if (message.value.isNotEmpty()) blue else Color.Gray
                )
            }
        },
        maxLines = 3,
